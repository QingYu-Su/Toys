...
func (r *router) handle(c *Context) {
	n, params := r.getRoute(c.Method, c.Path) // 查找匹配的路由
	if n != nil {                             // 如果找到匹配的路由
		c.Params = params                 // 设置路径参数
		key := c.Method + "-" + n.pattern // 生成唯一键，用于查找处理函数
		r.handlers[key](c)                // 调用对应的处理函数
	} else {
		c.String(http.StatusNotFound, "404 NOT FOUND: %s\n", c.Path) // 如果没有找到匹配的路由，返回404错误
	}
}
