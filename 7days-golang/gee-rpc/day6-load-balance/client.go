...
// NewHTTPClient 创建一个通过 HTTP CONNECT 方法建立连接的 RPC 客户端。
// 它首先发送一个 HTTP CONNECT 请求到指定的 RPC 路径，
// 然后等待服务器的响应。如果响应状态为 "200 Connected to Gee RPC"，
// 则使用该连接创建一个普通的 RPC 客户端。
func NewHTTPClient(conn net.Conn, opt *Option) (*Client, error) {
	// 向服务器发送 HTTP CONNECT 请求
	_, _ = io.WriteString(conn, fmt.Sprintf("CONNECT %s HTTP/1.0\n\n", defaultRPCPath))

	// 读取服务器的 HTTP 响应
	resp, err := http.ReadResponse(bufio.NewReader(conn), &http.Request{Method: "CONNECT"})
	if err == nil && resp.Status == connected { // 如果响应状态正确
		return NewClient(conn, opt) // 使用连接创建 RPC 客户端
	}
	if err == nil { // 如果没有读取错误，但响应状态不正确
		err = errors.New("unexpected HTTP response: " + resp.Status) // 返回错误
	}
	return nil, err // 返回 nil 和错误
}

// DialHTTP 通过 HTTP CONNECT 方法连接到指定的 RPC 服务器，并返回一个客户端实例。
// 它是一个高级接口，支持超时和默认选项。
func DialHTTP(network, address string, opts ...*Option) (*Client, error) {
	return dialTimeout(NewHTTPClient, network, address, opts...) // 调用 dialTimeout 函数
}

// XDial 是一个通用的 RPC 客户端拨号函数，支持多种协议。
// rpcAddr 的格式为 "protocol@addr"，例如 "http@localhost:8080" 或 "tcp@localhost:8080"。
// 根据协议类型，XDial 会调用相应的拨号函数（如 DialHTTP 或 Dial）来建立连接。
func XDial(rpcAddr string, opts ...*Option) (*Client, error) {
	// 将 rpcAddr 按照 "@" 分割为协议和地址
	parts := strings.Split(rpcAddr, "@")
	if len(parts) != 2 {
		// 如果格式不正确，返回错误
		return nil, fmt.Errorf("rpc client err: wrong format '%s', expect protocol@addr", rpcAddr)
	}
	protocol, addr := parts[0], parts[1] // 提取协议和地址

	// 根据协议类型选择拨号函数
	switch protocol {
	case "http":
		// 如果协议是 "http"，调用 DialHTTP 函数
		return DialHTTP("tcp", addr, opts...)
	default:
		// 其他协议（如 tcp、unix 等）直接调用 Dial 函数
