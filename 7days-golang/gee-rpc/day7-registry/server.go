...
// 定义了一些常量，用于 HTTP RPC 服务。
const (
	connected        = "200 Connected to Gee RPC" // HTTP CONNECT 响应消息
	defaultRPCPath   = "/_geeprc_"                // 默认的 RPC 路径
	defaultDebugPath = "/debug/geerpc"            // 默认的调试路径
)

// ServeHTTP 实现了 http.Handler 接口，用于处理 HTTP RPC 请求。
// 它支持通过 HTTP CONNECT 方法建立长连接，并将请求转发到 ServeConn 方法。
func (server *Server) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	if req.Method != "CONNECT" {
		// 如果请求方法不是 CONNECT，返回 405 Method Not Allowed 错误
		w.Header().Set("Content-Type", "text/plain; charset=utf-8")
		w.WriteHeader(http.StatusMethodNotAllowed)
		_, _ = io.WriteString(w, "405 must CONNECT\n")
		return
	}
	// 使用 http.Hijacker 接管连接
	conn, _, err := w.(http.Hijacker).Hijack()
	if err != nil {
		log.Print("rpc hijacking ", req.RemoteAddr, ": ", err.Error())
		return
	}
	// 发送 HTTP CONNECT 响应消息
	_, _ = io.WriteString(conn, "HTTP/1.0 "+connected+"\n\n")
	// 将连接传递给 ServeConn 方法处理
	server.ServeConn(conn)
}

// HandleHTTP 注册 HTTP RPC 处理器和调试页面处理器。
// 它将默认的 RPC 路径（/_geeprc_）映射到 Server 的 ServeHTTP 方法，
// 并将调试路径（/debug/geerpc）映射到调试页面处理器。
...
// HandleHTTP 是 DefaultServer 的 HandleHTTP 方法的简化调用。
// 它允许用户直接为默认服务器注册 HTTP RPC 处理器。
func HandleHTTP() {
