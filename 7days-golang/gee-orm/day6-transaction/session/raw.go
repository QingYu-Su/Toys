...
type Session struct {
	...
	refTable *schema.Schema  // 引用的表结构，包含表的字段、类型等信息
	clause   clause.Clause   // SQL 片段管理器，用于动态构建 SQL 语句的不同部分
	sql      strings.Builder // 用于构建最终的 SQL 语句
...
// Clear 用于清空当前 Session 的 SQL 构建状态，以便重新构建新的 SQL 语句。
func (s *Session) Clear() {
	s.sql.Reset()              // 清空 sql.Builder，重置 SQL 语句的构建状态
	s.sqlVars = nil            // 清空 SQL 参数列表
	s.clause = clause.Clause{} // 重置 clause.Clause，清空所有 SQL 子句及其参数
}
